...
function updateBenchmarkOverlay() {
  ...
  const isPretext = state.renderEngine === 'pretext';
  
  const layoutVal = isPretext ? b.layoutTime : b.domLayoutTime;
  const prepareVal = b.prepareTime;
  const measureVal = b.measureTime;

  overlay.innerHTML = `
    <div class="benchmark-title" style="margin-bottom: 12px; font-size: 0.75rem; letter-spacing: 0.15em; font-weight: 800;">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="var(--accent-amber)" style="margin-right: 8px;"><path d="M13 3h-2v10h2V3zm4.83 2.17l-1.42 1.42C17.99 7.86 19 9.81 19 12c0 3.87-3.13 7-7 7s-7-3.13-7-7c0-2.19 1.01-4.14 2.58-5.42L6.17 5.17C4.23 6.82 3 9.26 3 12c0 4.97 4.03 9 9 9s9-4.03 9-9c0-2.74-1.23-5.18-3.17-6.83z"/></svg>
      Benchmarks
    </div>
    
    <div class="benchmark-row" style="opacity: ${prepareVal > 0 ? 1 : 0.4}">
      <span class="benchmark-label">prepare()</span>
      <span class="benchmark-value">${prepareVal.toFixed(3)}ms</span>
    </div>
    <div class="benchmark-row">
      <span class="benchmark-label">layout()</span>
      <span class="benchmark-value">${layoutVal.toFixed(3)}ms</span>
    </div>
    <div class="benchmark-row">
      <span class="benchmark-label">analysis</span>
      <span class="benchmark-value">${b.analysisTime.toFixed(3)}ms</span>
    </div>
    <div class="benchmark-row">
      <span class="benchmark-label">measure</span>
      <span class="benchmark-value">
        ${measureVal.toFixed(3)}ms
      </span>
    </div>
